...
ax.set_xlim([-max(abs(posicao_inicial), abs(posicao_final)) * 15 /10, max(abs(posicao_inicial), abs(posicao_final)) * 15 / 10])  # amplia um intervalo dado no x
ax.set_ylabel('Time')
ax.set_xlabel('Space')
...
if(velocidade != 0):
     ...
     ax.set_ylim([-1, ((posicao_final - posicao_inicial) / velocidade) + 10])
else:
     ax.set_ylim([-1, 10])  # amplia um intervalo dado no y

# frames: distancia que ele vai percorrer
# interval: velocidade
anim = animation.FuncAnimation(fig, _update_plot, fargs=(
    fig, scat), frames=lista, interval=60)
plt.show()
